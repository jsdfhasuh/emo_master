...
> 本次交付：仅修订本计划文档；不实现功能、不新增实际图标、不调整依赖或发布配置。  
...
本次只修订设计和验收契约。下述安全测试、GUI 测试、超时测试、发布包测试均是后续实施要求，不是本次已完成的实测结果。
...
以下结论以文首代码基线为准；计划提交 `7081e25` 只新增计划，未修改这些实现。相关文件可通过附录 A 定位。
...
| Designer `ui/icon_map.py` | `getOperatorGlyph()` 只有六个字符映射 | 保留兜底，增加统一图片显示服务 |
| Designer `ui/operator_bubble.py` | 把字符图标拼进按钮文本 | 有图片时使用真正图标，标题保持纯文本 |
...
以上为待审阅设计，并非已实现行为。容量、超时、关闭预算和结构限额是初始设计值，不是性能实测结论；P0 阶段需冻结实现常量及测试表，调整时同步更新本计划。
...
当前目录身份对应的图片已校验且可渲染（包括有效内存缓存）
    → 显示自定义图片
否则
    → 使用 getOperatorGlyph(iconKey) 的现有兜底
iconKey 不认识
    → 使用 default 兜底
```
...
新增“小尺寸、小文件、压缩附加数据膨胀”和“合法尺寸但 IDAT 超额解压”夹具。这是待编写测试，不引用未随仓库提交并复现的样例数值作为通过证据。
...
P0 固定经 Python 3.10 验证的依赖版本，P1 同步维护 `pyproject.toml`、`requirements.txt` 等实际依赖入口。缺少安全解析器时 SVG 独立降级，诊断 `E_ICON_VALIDATOR_UNAVAILABLE`；PNG 和有效算子仍可使用。正式包缺少此依赖则不能通过 SVG 验收。
...
本次文档修订不生成这些图片。全量配图随后独立规划：导出实际注册清单，按用途分组，记录资源和许可来源，允许基础图形复用但须易区分。外部图标需记录适用许可，不引入来源不明资源。
...
当前审批记录：`PENDING`。同意修订并推送文档不等于同意开始实现；没有用户明确批准记录，不改为 `IMPLEMENTATION_READY`，不启动功能代码或全量图标制作。

## 附录 A：源码依据

以下路径相对本计划目录；当前实现事实以文首代码提交为准。实施前重新核对，不把旧架构文档的历史问题当成现状。
